...
func TestTypesInput(t *testing.T) {
	...
	filterTests := []jsonpathTest{
		{"filterPrimitive", `{ $.integers[0][?(@ == 1)] }`, types, `1`, false},
		{"filterPrimitiveNoMatch", `{ $.integers[0][?(@ == 2)] }`, types, ``, false},
		{"filterFieldOrKey", `{ $..value }`, types, `1 2.02 3.03 4.04 1 2.002 3 4.004`, false},
		{"filterStructField", `{ $['structs'][?(@.Type == "integer")] }`, types,
			`{"name":"one","value":1,"type":"integer"} {"name":"three","value":3,"type":"integer"}`, false},
		{"filterMapKey", `{ $['maps'][?(@.name == "one")] }`, types,
			`{"name":"one","value":1}`, false},
		{"filterJsonTag", `{ $..[?(@.name == "one")] }`, types,
			`{"name":"one","value":1} {"name":"one","value":1,"type":"integer"}`, false},
		{"filterRecursion", `{ $..[?(@.Type == "integer")] }`, types,
			`{"name":"one","value":1,"type":"integer"} {"name":"three","value":3,"type":"integer"}`, false},
		{"filterRecursionAlt", `{ $..[?(@['Type'] == "integer")] }`, types,
			`{"name":"one","value":1,"type":"integer"} {"name":"three","value":3,"type":"integer"}`, false},
		{"filterRecursionPrimitive", `{ $..[?(@ == "integer")] }`, types,
			`integer integer`, false},
		// recursively iterates and tests comparison against both integers and floats
		{"filterIntegerFloatComparison", `{ $..[?(@ >= 2)] }`, types,
			`2.2 3.3 4 2 3 4 2.02 3.03 4.04 2.002 3 4.004`, false},
	}

	// test with missing attributes/keys allowed
	testJSONPath(filterTests, true, t)
}
...
func TestStructInput(t *testing.T) {
	...
	storeTests := []jsonpathTest{
		...
		{"allstructsSlice", "{.Bicycle}", storeData, `[{"Color":"red","Price":19.95,"IsNew":true},{"Color":"green","Price":20.01,"IsNew":false}]`, false},
		{"allstructs", `{range .Bicycle[*]}{ @ }{ " " }{end}`, storeData, `{"Color":"red","Price":19.95,"IsNew":true} {"Color":"green","Price":20.01,"IsNew":false} `, false},
